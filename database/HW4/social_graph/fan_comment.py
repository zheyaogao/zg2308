...
class FanGraph(object):
    ...
    def create_comment(self, uni, comment, team_id=None, player_id=None):
        ...
        if team_id or player_id:
            ...
            print("Player id and team id are both None")
            return None
    ...
    def create_sub_comment(self, uni, origin_comment_id, comment):
        ...
        if c:
            ...
            n = Node("Comment", comment_id=comment_id, comment=comment)
            f = self.get_fan(uni)
            r1 = Relationship(n, "RESPONSE_TO", c)
            r2 = Relationship(f, "RESPONSE_BY", n)
            tx = self._graph.begin(autocommit=True)
            tx.create(n)
            tx = self._graph.begin(autocommit=True)
            tx.create(r1)
    ...
    def get_player_comments(self, player_id):
        ...
        [response_to:RESPONSE_TO*]-(sub_comment:Comment)-[response_by:RESPONSE_BY]-(sub_commenter:Fan) with  \
        player,on,comment,response_to,sub_comment,response_by,sub_commenter match (comment)-[comment_by:COMMENT_BY] \
        -(commenter:Fan) return player,on,comment,comment_by,commenter,response_to,sub_comment,response_by,sub_commenter'
        result = self._graph.run(q).data()
        return result
    ...
    def get_team_comments(self, team_id):
        ...
        [response_to:RESPONSE_TO*]-(sub_comment:Comment)-[response_by:RESPONSE_BY]-(sub_commenter:Fan) with  \
        team,on,comment,response_to,sub_comment,response_by,sub_commenter match (comment)-[comment_by:COMMENT_BY] \
        -(commenter:Fan) return team,on,comment,comment_by,commenter,response_to,sub_comment,response_by,sub_commenter'
        result = self._graph.run(q).data()
        return result
